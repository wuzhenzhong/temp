...
ABP已经把 **ASP.NET MVC Controllers** 集成到了 **Abp.Web.Mvc** 中，正如你一直那样做的，你可以使用常规的MVC Controllers。 [依赖注入](https://www.52abp.com/Wiki/abp-cn/latest/2.1ABP公共结构-依赖注入)可以解析常规的MVC Controllers。
...
异常是自动被处理的，会被记录到日志并且以一个适当的方式返回到客户端。详情请参考[异常处理](https://www.52abp.com/Wiki/abp-cn/latest/6.5ABP表现层-异常处理)。

#### 3. 包装返回内容

如果返回的类型是 **JsonResult** (或者异步Action的Task)ABP默认会将Action的返回内容进行包装。你可以改变这个，只要在Controller的Action上面使用特性：**WrapResult** 或者 **DontWrapResult**，详情请参考[Javascript函数库](https://www.52abp.com/Wiki/abp-cn/latest/6.6ABP表现层-Javascript函数库)

#### 4. 审计日志

如果派生自AbpController，那么日志会被自动记录。详情请参考[审计日志](https://www.52abp.com/Wiki/abp-cn/latest/4.7ABP应用层-审计日志)。

#### 5. 权限认证

你可以用 **AbpMvcAuthorize** 特性来阻止未授权的用户来访问你的Controllers和Actions。 示例如下：
...
AbpController也定义了一个快速检测权限的方法 **IsGranted**。 详情请参照[权限认证](https://www.52abp.com/Wiki/abp-cn/latest/4.4ABP应用层-权限认证)。
...
详情请参照[工作单元](https://www.52abp.com/Wiki/abp-cn/latest/3.4ABP领域层-工作单元)。
