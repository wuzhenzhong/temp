...
![img](https://raw.githubusercontent.com/52ABP/ABPCNDocs/V3.7.4/src/Wiki_abp_images/5.2.1.png)

**SimpleTaskSystem** 是资源文件的名字，SimpleTaskSystem.xml是默认的本地化语言. 当某个文本被请求的时候，ABP会从当前的XML资源文件中去取得本地化文本(通过 Thread.CurrentThread.**CurrentUICulture** 来找到当前所使用的语言)。如果当前所使用的语言不存在，那么将会使用默认的XML的语言资源。
...
我们应该在模块的 **PreInitialize** 方法中来配置，详情请参照[模块系统](https://www.52abp.com/Wiki/abp-cn/latest/1.3ABP总体介绍-模块系统)。ABP会从给定的目录中找到所有的XML文件并且注册这些本地化资源。
...
![img](https://raw.githubusercontent.com/52ABP/ABPCNDocs/V3.7.4/src/Wiki_abp_images/json-localization-source-files.png)
...
我们应该在模块的 **PreInitialize** 方法中来配置，详情请参照[模块系统](https://www.52abp.com/Wiki/abp-cn/latest/1.3ABP总体介绍-模块系统)。ABP会从给定的目录中找到所有的JSON文件并且注册这些本地化资源。
...
![img](https://raw.githubusercontent.com/52ABP/ABPCNDocs/V3.7.4/src/Wiki_abp_images/5.2.2.png)

MyTexts.resx包含了默认的语言配置文件，MyTexts.tr.resx包含了土耳其语言配置文件。当打开MyTexts.resx:

![img](https://raw.githubusercontent.com/52ABP/ABPCNDocs/V3.7.4/src/Wiki_abp_images/5.2.3.png)
...
- **AbpUserRequestCultureProvider** (ABP的提供器)：如果通过[IAbpSession](https://www.52abp.com/Wiki/abp-cn/latest/2.2ABP公共结构-会话管理.md)获知当前用户(用户已经登录)，并在此之前明确的选择了一种语言(并保存该选择到[ISettingManager](https://www.52abp.com/Wiki/abp-cn/latest/2.5ABP公共结构-设置管理))，那么使用该用户首选的语言。如果是已知用户(用户已经登录)，但是没有选择任何语言，而 **.AspNetCore.Culture** cookie或者header有值，那么使用该信息来设置用户的语言设置，并使用该值作为当前语言。如果用户未知(用户未登录)，这个提供器不会做任何事情。
...
- 如果没有找到，尝试取得默认的culture设置(设置的名称是：**"Abp.Localization.DefaultLanguageName"**，这个常量定义在：Abp.Localization.LocalizationSettingNames.DefaultLanguage，可以使用[设置管理](https://www.52abp.com/Wiki/abp-cn/latest/2.5ABP公共结构-设置管理)来改变它)。
...
如果你需要在某个[应用服务](https://www.52abp.com/Wiki/abp-cn/latest/4.1ABP应用层-应用服务)，MVC Controller，Razor View 或者其它派生自 **AbpServiceBase**的类中使用本地化，那么你可以使用 **L** 方法。
...
![img](https://raw.githubusercontent.com/52ABP/ABPCNDocs/V3.7.4/src/Wiki_abp_images/6.3.5.png)
