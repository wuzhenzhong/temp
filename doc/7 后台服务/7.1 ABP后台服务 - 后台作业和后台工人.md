...
后台作业必须使用[依赖注入](https://www.52abp.com/Wiki/abp-cn/latest/2.1ABP公共结构-依赖注入)进行注册，而实现 **ITransientDependency** 是最简单的方式。
...
**BackgroundJobManager** 默认实现了IBackgroundJobManager接口。它能够被其它的后台作业提供者给替换掉(如：[集成Hangfire](https://www.52abp.com/Wiki/abp-cn/latest/7.2ABP后台服务-集成Hangfire))。在 **BackgroundJobManager** 中默认定义了一些信息：
...
BackgroundJobManager 默认是需要数据存储来保存和检索作业的。如果你没有实现 **IBackgroundJobStore**，那么它会使用 **InMemoryBackgroundJobStore** 来存储作业，当然作业不会被持久化到数据库中。你可以简单的实现它来存储作业到数据库中或者你可以在[module-zero](https://www.52abp.com/Wiki/abp-cn/latest//Markdown/AbpZero/1.1ABPZero-概述)直接使用，因为它已被实现。
...
由于后台作业管理默认应该重试失败的作业，它会处理(并记录日志)所有的异常。如果你想当异常发生的时候被通知，你可以创建事件处理来处理 [AbpHandledExceptionData](https://www.52abp.com/Wiki/abp-cn/latest/6.5ABP表现层-异常处理)。后台会触发这个事件并且附上一个 **BackgroundJobException** 异常对象，该对象包裹了真实的异常(对该异常获取 **InnerException**)。

#### 6. 集成Hangfire

后台作业管理者可以被其他后台作业管理者替换。详情请参考[集成Hangfire](https://www.52abp.com/Wiki/abp-cn/latest/7.2ABP后台服务-集成Hangfire)
...
这是一个真实性的代码，你可以在[module-zero](https://www.52abp.com/Wiki/abp-cn/latest//Markdown/AbpZero/1.1ABPZero-概述)找到。
...
ABP的后台worker系统是相当简单的。如上所述，除了周期性的运行workes，它没有调度系统。如果你想要更高级的调度系统功能，建议你使用 [Quartz](https://www.52abp.com/Wiki/abp-cn/latest/7.3ABP后台服务-集成Quartz)或者其他类库。
