...
打开 `lavas-user/lavas.config.js` 配置文件的`build` 段的 `publicPath` 以及 `router` 段的 `base`，均修改为 `/user/` (**不要遗漏最后的** `**/**` **！**)，如下：
...
Lavas 为开发者提供了 `alias` 配置项([文档](https://lavas.baidu.com/guide/v2/advanced/build-config#alias))，修改 `/lavas.config.js` 即可。
...
```javascript
123456789101112131415161718192021222324252627282930313233343536373839404142434445464748495051525354555657585960const path = require('path');
...
文件的上半部分对 SPA 进行处理，核心是把 `/user` 开头的路由转发到 lavas-user 的入口 `lavas-user/dist/index.html` 上。其中还涉及到一个 URL 结尾 `/` 的小问题，我们在[最后](https://lavas.baidu.com/guide/v2/advanced/multi-lavas#express-处理-spa-路由的小问题-扩展)进行叙述。SPA 部分的最后是启动 express 服务器并监听端口，但因为 SSR 部分包含异步操作，因此 **如果项目包含 SSR 部分，则这里可以注释，由 SSR 部分负责启动**。
...
```javascript
123456789101112131415lavas-project
├── lavas-user/
│   ├── dist
│   │   ├── index.html
...
```javascript
12345678910// fix trailing slash (/user -> /user/)
app.use('/', function (req, res, next) {
    let requestUrl = req.url.replace(/\?.+?$/, '');
...
```javascript
1234rewriteTarget = options.index || '/index.html';
...
```javascript
1234app.use('/user', historyMiddleware({
    htmlAcceptHeaders: ['text/html'],
    disableDotRule: false // ignore paths with dot inside
}))
```
