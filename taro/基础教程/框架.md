...
通常入口文件会包含一个 `config` 配置项，这个配置是整个应用的全局的配置，配置规范基于微信小程序的[全局配置](https://developers.weixin.qq.com/miniprogram/dev/framework/config.html#全局配置)进行制定，所有平台进行统一。
...
| componentWillPreload | [预加载](https://taro-docs.jd.com/taro/docs/best-practice.html#预加载) | ✔️          | ✘          | ✘              | ✘                  | ✘    | ✘    |
